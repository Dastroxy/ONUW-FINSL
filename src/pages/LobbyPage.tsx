...
const LobbyPage: React.FC<Props> = ({ game, me }) => {
  ...
  if (game.phase === GamePhase.LOBBY) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative">
         <div className="absolute inset-0 bg-gradient-to-b from-[#0a0e1a] via-[#0d1228] to-[#080b14] pointer-events-none"></div>
         ...
         <div className="relative z-10 w-full max-w-sm glass-panel rounded-2xl p-6 mb-8 text-center shadow-[0_0_40px_rgba(232,213,163,0.06),_0_0_80px_rgba(196,93,44,0.04)] animate-fade-in-up" style={{ border: '1px solid rgba(232, 213, 163, 0.15)' }}>
            <h3 className="text-moon/60 text-sm uppercase tracking-widest font-bold mb-2 font-display">Room Code</h3>
            <div className="flex items-center justify-center gap-3 mb-4">
               <span className="text-5xl font-mono font-bold text-moon tracking-wider" style={{ textShadow: '0 0 20px rgba(232, 213, 163, 0.4), 0 0 40px rgba(196, 93, 44, 0.25)' }}>
                 {game.id}
               </span>
            </div>
            <button 
               onClick={copyToClipboard}
               className={`text-xs px-3 py-1.5 rounded-full border transition-all duration-300 ${copied ? 'bg-green-500/20 border-green-500 text-green-300' : 'bg-white/5 border-moon/20 text-moon/70 hover:bg-primary/10 hover:border-primary/40'}`}
            >
         ...
         <div className="relative z-10 w-full max-w-sm mb-8 flex-1 overflow-y-auto">
             <div className="flex justify-between items-end mb-4 px-2">
                 <h2 className="text-xl font-bold text-moon font-display" style={{ textShadow: '0 0 10px rgba(232, 213, 163, 0.2)' }}>Lobby</h2>
             ...
             <ul className="space-y-2">
                {players.map((p) => (
                  <li key={p.id} className="bg-forest/70 border border-moon/10 p-3 rounded-lg flex items-center gap-3 shadow-[0_2px_15px_rgba(0,0,0,0.3)] backdrop-blur-sm animate-fade-in hover:border-primary/30 transition-colors">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${p.isHost ? 'bg-primary text-white shadow-[0_0_12px_rgba(196,93,44,0.5)]' : 'bg-bark text-moon/70 border border-moon/10'}`}>
                         {p.name.charAt(0).toUpperCase()}
                      </div>
                      <span className="text-moon/90 font-medium">{p.name}</span>
                      {p.isHost && <span className="ml-auto text-[10px] bg-primary/20 text-primary-light border border-primary/30 px-2 py-0.5 rounded shadow-[0_0_8px_rgba(196,93,44,0.2)]">HOST</span>}
                  </li>
                ))}
                {Array.from({ length: Math.max(0, MIN_PLAYERS - players.length) }).map((_, i) => (
                   <li key={`empty-${i}`} className="border-2 border-dashed border-forest/60 p-3 rounded-lg flex items-center gap-3 opacity-40">
                       <div className="w-8 h-8 rounded-full bg-background border border-forest/40"></div>
                       <span className="text-moon/20 text-sm italic">Waiting for players...</span>
                   </li>
                ))}
             </ul>
         </div>
             ...
             {isHost ? (
               <button 
                 ...
                 className={`w-full py-4 rounded-xl font-bold text-lg shadow-lg transition-all duration-300
                    ${canStartSeating 
                      ? 'bg-gradient-to-r from-primary to-primary-light hover:from-primary/90 hover:scale-[1.02] text-white shadow-[0_0_30px_rgba(196,93,44,0.3)] border border-primary/30' 
               ...
               <div className="text-center p-4 glass-panel rounded-xl animate-pulse" style={{ border: '1px solid rgba(196, 93, 44, 0.15)' }}>
  ...
  if (game.phase === GamePhase.SEATING) {
     ...
     return (
        <div className="p-4 flex flex-col items-center min-h-screen relative">
           <div className="absolute inset-0 bg-gradient-to-b from-[#0a0e1a] via-[#10162c] to-[#080b14] pointer-events-none"></div>
           ...
           <header className="mb-6 text-center pt-4 relative z-10">
               <h2 className="text-3xl font-display font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-primary to-moon mb-2" style={{ filter: 'drop-shadow(0 0 12px rgba(196, 93, 44, 0.3))' }}>Select Your Seat</h2>
           ...
           <div className="mt-4 w-full max-w-md glass-panel p-4 rounded-xl mb-24 relative z-10" style={{ border: '1px solid rgba(232, 213, 163, 0.1)' }}>
             ...
             <ul className="grid grid-cols-2 gap-2 text-sm max-h-40 overflow-y-auto">
               {players.map(p => (
                 <li key={p.id} className={`px-3 py-2 rounded border flex items-center justify-between transition-colors ${p.seatId !== null ? 'bg-primary/10 border-primary/50 text-moon shadow-[0_0_12px_rgba(196,93,44,0.2)]' : 'bg-surface/60 border-forest/50 text-moon/30 opacity-60'}`} style={p.seatId !== null ? { boxShadow: '0 0 12px rgba(232, 213, 163, 0.06), inset 0 1px 0 rgba(232, 213, 163, 0.05)' } : {}}>
                    <span className="truncate max-w-[100px]">{p.name}</span>
                    {p.seatId !== null ? 
                      <span className="text-[10px] font-bold bg-primary px-1.5 py-0.5 rounded text-white ml-2 shadow-[0_0_6px_rgba(196,93,44,0.4)]">#{p.seatId + 1}</span> :
           ...
           {isHost ? (
              <div className="fixed bottom-6 left-0 right-0 px-6 flex justify-center z-50">
                  <button 
                    ...
                    className={`w-full max-w-md py-4 rounded-xl font-bold text-lg shadow-2xl transition-all duration-300 relative overflow-hidden group
                        ${canProceedToRoles 
                            ? 'bg-gradient-to-r from-primary to-primary-light hover:from-primary/90 hover:to-primary-light/90 text-white shadow-[0_0_35px_rgba(196,93,44,0.4)] transform hover:scale-[1.02] border border-moon/15' 
                            : 'bg-bark text-moon/30 cursor-not-allowed border border-forest'}`}
                    onClick={() => advanceToRoles(game.id)}
                    style={canProceedToRoles ? { boxShadow: '0 0 35px rgba(196, 93, 44, 0.4), 0 0 60px rgba(232, 213, 163, 0.08)' } : {}}
                  >
             ...
             <div className="fixed bottom-6 left-0 right-0 px-6 text-center z-50">
                <div className="bg-[#0a0e1a]/80 backdrop-blur-md px-6 py-3 rounded-full inline-block border border-moon/10 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
  ...
  if (game.phase === GamePhase.ROLES) {
     ...
     return (
        <div className="flex flex-col h-screen" style={{ background: 'linear-gradient(180deg, #080b14 0%, #0d1228 50%, #0a0e1a 100%)' }}>
          <header className="flex justify-between items-center p-4 border-b border-moon/8 sticky top-0 z-30 shadow-[0_4px_30px_rgba(0,0,0,0.5)]" style={{ background: 'rgba(10, 14, 26, 0.85)', backdropFilter: 'blur(20px)' }}>
             <div>
                <h2 className="text-lg font-display font-bold text-moon" style={{ textShadow: '0 0 10px rgba(232, 213, 163, 0.2)' }}>Select Roles</h2>
                                 ...
                                 return (
                                     <div 
                                         ...
                                         className={`
                                             ...
                                             backdrop-blur-sm
                                             ${isSelected 
                                                 ? 'border-moon/40 bg-primary/20 shadow-[0_0_25px_rgba(196,93,44,0.3),_0_0_50px_rgba(232,213,163,0.08)] scale-105 z-10 ring-2 ring-primary/40' 
                                                 : 'border-white/8 bg-white/[0.03] hover:bg-primary/10 hover:border-primary/40 hover:shadow-[0_10px_30px_rgba(196,93,44,0.15)] hover:-translate-y-1'}
                                         `}
                                     >
                                         ...
                                         <div className="flex-1 flex flex-col items-center justify-center p-2 text-center relative z-10">
                                             <div className={`mb-2 transition-all duration-300 ${isSelected ? 'scale-110 drop-shadow-[0_0_12px_rgba(232,213,163,0.5)]' : 'group-hover:scale-110 group-hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]'} grayscale-[0.3] group-hover:grayscale-0`}>
                                         ...
                                         <div className={`
                                            absolute top-2 right-2 w-5 h-5 bg-primary rounded-full shadow-[0_0_10px_rgba(196,93,44,0.5)] flex items-center justify-center text-[10px] text-white transition-all duration-300
          ...
          {isHost ? (
             <div className="fixed bottom-0 left-0 right-0 p-6 z-40" style={{ background: 'linear-gradient(to top, #080b14 0%, rgba(8, 11, 20, 0.95) 60%, transparent 100%)' }}>
               <button 
                 ...
                 className={`
                    ...
                    ${selectedRoles.length === requiredRolesCount
                        ? 'bg-gradient-to-tr from-accent to-evil border-2 border-accent/60 text-white shadow-[0_8px_30px_rgba(139,26,26,0.4),_0_0_50px_rgba(232,213,163,0.06)] hover:shadow-[0_8px_40px_rgba(139,26,26,0.6)] hover:brightness-110 hover:scale-[1.02] active:scale-95'
                        : 'bg-bark text-moon/30 cursor-not-allowed border border-forest'}
                 `}
               >
             ...
             <div className="fixed bottom-0 left-0 right-0 p-6 z-40 text-center" style={{ background: 'linear-gradient(to top, #080b14 0%, rgba(8, 11, 20, 0.95) 60%, transparent 100%)' }}>
                 <div className="text-primary font-bold animate-pulse mb-2" style={{ textShadow: '0 0 15px rgba(196, 93, 44, 0.3)' }}>HOST IS SELECTING ROLES</div>
  ...
  return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative overflow-hidden">
         <div className="absolute inset-0 bg-gradient-to-b from-[#0a0e1a] via-[#10162c] to-[#080b14] pointer-events-none"></div>
         <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_rgba(100,60,200,0.1)_0%,_rgba(196,93,44,0.05)_30%,_transparent_60%)] pointer-events-none"></div>
         <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_rgba(232,213,163,0.04)_0%,_transparent_50%)] pointer-events-none"></div>
         
         <h2 className="text-3xl font-display font-bold mb-8 text-moon relative z-10 tracking-wide" style={{ textShadow: '0 0 20px rgba(232, 213, 163, 0.3), 0 2px 10px rgba(0, 0, 0, 0.5)' }}>Your Role</h2>
         
         <div className="z-20">
            <RoleCard role={me.originalRole} flipped={cardFlipped} onClick={() => setCardFlipped(!cardFlipped)} size="lg" className="card-glow shadow-[0_0_40px_rgba(196,93,44,0.2),_0_0_80px_rgba(100,60,200,0.08)]" />
         </div>
         ...
         <div className={`mt-8 text-center max-w-xs relative z-10 transition-all duration-500 ${cardFlipped ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
             <div className="mb-4 flex justify-center">
                 <RoleIcon role={me.originalRole} className="w-16 h-16 drop-shadow-[0_0_20px_rgba(196,93,44,0.4)]" />
             </div>
             <h3 className="text-2xl font-display font-bold text-primary mb-2" style={{ textShadow: '0 0 15px rgba(196, 93, 44, 0.4)' }}>{ROLE_METADATA[me.originalRole].name}</h3>
             <p className="text-moon/70 text-sm leading-relaxed glass-panel p-4 rounded-xl shadow-xl" style={{ border: '1px solid rgba(232, 213, 163, 0.1)' }}>
         ...
         {(() => {
            ...
            return (
              <div className="mt-10 flex flex-col items-center gap-3 relative z-30">
                {isDealReady ? (
                  <div className="bg-primary/15 border border-moon/20 text-primary-light px-8 py-3 rounded-full font-bold text-lg tracking-wider animate-pulse shadow-[0_0_20px_rgba(232,213,163,0.08)]">
                    READY
                  </div>
                ) : (
                  <button
                    onClick={() => toggleDealReady(game.id, me.id)}
                    className="bg-gradient-to-r from-accent to-evil hover:from-accent/90 hover:to-evil/90 text-white px-10 py-4 rounded-full font-display font-bold text-lg shadow-[0_0_35px_rgba(139,26,26,0.5),_0_0_60px_rgba(232,213,163,0.06)] transition-all transform hover:scale-105 active:scale-95 tracking-wider border border-moon/10"
                  >
                    READY
                  </button>
                )}
